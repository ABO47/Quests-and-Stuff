package com.abo47.questsandstuff.quest.editor.group;
...
public final class GroupEditService {
    ...
    public GroupEditService(EditorSessionService owner) {
        this.owner = owner;
    }
    ...
    public void deleteGroup(ServerPlayer player, String groupName) {
        ...
        for (QuestDefinition quest : new ArrayList<>(owner.definitionStore().quests().values())) {
            ...
            Map<String, GroupDef> map = new HashMap<>(quest.display().groups());
            map.remove(group);
            if (map.isEmpty()) {
    ...
    public void renameGroup(ServerPlayer player, String fromName, String toName) {
        ...
        for (QuestDefinition quest : new ArrayList<>(owner.definitionStore().quests().values())) {
            ...
            Map<String, GroupDef> map = new HashMap<>(quest.display().groups());
            GroupDef view = map.remove(from);
            map.put(to, view == null ? GroupDef.DEFAULT : view);
            owner.definitionStore().upsert(withGroups(quest, map));
        }
        groups.set(index, to);
        owner.definitionStore().setGroupOrder(groups);
