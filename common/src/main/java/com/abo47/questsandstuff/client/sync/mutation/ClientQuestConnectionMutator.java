...
final class ClientQuestConnectionMutator {
    private ClientQuestConnectionMutator() {
    }
    ...
    static void setQuestPrerequisiteLocal(String questId, String prerequisiteId, boolean add) {
        ...
        quest.put(SyncKeys.Quest.PREREQUISITES, next);
        if (!add) {
    ...
    static void setConnectionColorLocal(String questId, String prerequisiteId, int color) {
        ...
        CompoundTag colors = target.quest().getCompound(SyncKeys.Quest.CONNECTION_COLORS).copy();
        colors.putInt(target.metadataKey(), color);
        target.quest().put(SyncKeys.Quest.CONNECTION_COLORS, colors);
    }
    ...
    static void setConnectionModeLocal(String questId, String prerequisiteId, boolean gridMode) {
        ...
        CompoundTag modes = target.quest().getCompound(SyncKeys.Quest.CONNECTION_MODES).copy();
        if (gridMode) {
            modes.putString(target.metadataKey(), QuestConnectionMode.GRID.serializedName());
        } else {
            modes.remove(target.metadataKey());
        }
        target.quest().put(SyncKeys.Quest.CONNECTION_MODES, modes);
    }
    ...
    static void setConnectionHiddenLocal(String questId, String prerequisiteId, boolean hidden) {
        ...
        ListTag next = removeString(target.quest().getList(SyncKeys.Quest.HIDDEN_CONNECTIONS, Tag.TAG_STRING), target.metadataKey());
        if (hidden) {
            next.add(StringTag.valueOf(target.metadataKey()));
        }
        target.quest().put(SyncKeys.Quest.HIDDEN_CONNECTIONS, next);
    }
    ...
    static void removeQuestReferences(String questId) {
        ClientQuestState.forEachQuest(quest -> {
            ...
            if (changed) {
                quest.put(SyncKeys.Quest.PREREQUISITES, filtered);
    ...
    static void refreshLocalUnlockState(CompoundTag quest) {
        ...
        for (int i = 0; i < prerequisites.size(); i++) {
            ...
            if (prerequisite == null || !prerequisite.getBoolean(SyncKeys.Quest.COMPLETED)) {
    ...
    static void removeConnectionMetadata(CompoundTag quest, String prerequisiteId) {
        ...
        CompoundTag colors = quest.getCompound(SyncKeys.Quest.CONNECTION_COLORS).copy();
        colors.remove(key);
        quest.put(SyncKeys.Quest.CONNECTION_COLORS, colors);
        CompoundTag modes = quest.getCompound(SyncKeys.Quest.CONNECTION_MODES).copy();
        modes.remove(key);
        quest.put(SyncKeys.Quest.CONNECTION_MODES, modes);
        ...
        quest.put(SyncKeys.Quest.HIDDEN_CONNECTIONS, removeString(quest.getList(SyncKeys.Quest.HIDDEN_CONNECTIONS, Tag.TAG_STRING), key));
    }
