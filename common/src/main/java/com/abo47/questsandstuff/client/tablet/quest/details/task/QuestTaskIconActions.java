...
final class QuestTaskIconActions {
    private QuestTaskIconActions() {
    }

    static String taskIcon(String questId, String id, boolean task) {
        ...
        return task ? QuestTaskDisplayText.taskIcon(json) : QuestTaskDisplayText.rewardIcon(json);
    }

    static boolean isEntityTaskIcon(String questId, String id, boolean task) {
        return EntityPreviewRenderer.isEntityAsset(taskIcon(questId, id, task));
    }

    static void putTaskIcon(Player player, String questId, String id, String icon, boolean task) {
        putTaskIcon(player, questId, id, icon, task, true);
    }

    static void putTaskIcon(Player player, String questId, String id, String icon, boolean task, boolean sync) {
        CompoundTag quest = ClientQuestStateFacade.quest(questId);
        CompoundTag entries = quest.getCompound(task ? "tasks" : "rewards");
        CompoundTag entry = entries.getCompound(id);
