...
final class QuestTaskPickerApplyActions {
    private QuestTaskPickerApplyActions() {
    }
    ...
    static void applyIconPick(Player player, TabletUiState state, String entry) {
        ...
        if (!ModalTargetState.requireParts("task_icon", parsed, 4)) {
            return;
        }
        ...
        if (parsed.isTaskItem()) {
            ...
            JsonObject json = TaskJsonFactory.simpleTask(id, type, entry, entry);
            EditorQuestCommandClient.putQuestTaskJson(player, questId, json.toString());
            ...
            if (!entityId.isBlank()) {
                JsonObject json = TaskJsonFactory.simpleTask(id, type, entityId, "");
                EditorQuestCommandClient.putQuestTaskJson(player, questId, json.toString());
            ...
            QuestTaskIconActions.putTaskIcon(player, questId, id, entry, true);
            ...
            if (FluidIconCodec.isFluidIcon(entry)) {
                JsonObject json = fluidItemJson(id, type, entry);
                QuestTaskRewardEditActions.preserveRewardSelectableFlag(questId, id, json);
            ...
            QuestTaskIconActions.putTaskIcon(player, questId, id, entry, false);
    ...
    static void applyInventoryItemPick(Player player, TabletUiState state, ItemStack stack) {
        ...
        if (parsed.isQuestIcon() && ModalTargetState.requireParts("inventory_quest_icon", parsed, 2)) {
            EditorQuestCommandClient.runQuestIconAction(player, parsed.questId(), icon);
            state.questDetails.questDetailsPickTarget = "";
        ...
        if (parsed.isTaskIcon() && ModalTargetState.requireParts("inventory_task_icon", parsed, 3)) {
            QuestTaskIconActions.putTaskIcon(player, parsed.questId(), parsed.entryId(), icon, true);
            state.questDetails.questDetailsPickTarget = "";
        ...
        if (parsed.isRewardIcon() && ModalTargetState.requireParts("inventory_reward_icon", parsed, 3)) {
            QuestTaskIconActions.putTaskIcon(player, parsed.questId(), parsed.entryId(), icon, false);
            state.questDetails.questDetailsPickTarget = "";
        ...
        if (!ModalTargetState.requireParts("inventory_task_item", parsed, 4)) {
            return;
        }
        if (parsed.isTaskInventoryItem()) {
            ...
            json.addProperty("collection", "automatic");
            EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
            state.questDetails.questDetailsPickTarget = "";
        ...
        if (parsed.isRewardInventoryItem()) {
            ...
            json.addProperty("nbt", stack.hasTag() ? stack.getTag().toString() : "");
            QuestTaskRewardEditActions.preserveRewardSelectableFlag(parsed.questId(), parsed.entryId(), json);
    ...
    static void applyBiomePick(Player player, TabletUiState state, String biome) {
        ...
        if (!ModalTargetState.requireParts("task_biome", parsed, 4) || !parsed.isTaskBiome()) {
            return;
        }
        JsonObject json = TaskJsonFactory.simpleTask(parsed.entryId(), parsed.type(), biome, "biome");
        EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
        state.questDetails.questDetailsPickTarget = "";
    ...
    static void applyAdvancementPick(Player player, TabletUiState state, String advancement) {
        ...
        if (!ModalTargetState.requireParts("task_advancement", parsed, 4) || !parsed.isTaskAdvancement()) {
            return;
        }
        JsonObject json = TaskJsonFactory.simpleTask(parsed.entryId(), parsed.type(), advancement.trim(), "trophy");
        EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
        state.questDetails.questDetailsPickTarget = "";
    ...
    static void applyRecipePick(Player player, TabletUiState state, String recipe) {
        ...
        if (!ModalTargetState.requireParts("task_recipe", parsed, 4) || !parsed.isTaskRecipe()) {
            return;
        }
        String recipeTarget = recipe.trim();
        JsonObject json = TaskJsonFactory.simpleTask(parsed.entryId(), parsed.type(), recipeTarget, QuestTaskIconActions.recipeIcon(recipeTarget));
        EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
        state.questDetails.questDetailsPickTarget = "";
    ...
    static void applyStructurePick(Player player, TabletUiState state, String structure) {
        ...
        if (!ModalTargetState.requireParts("task_structure", parsed, 4) || !parsed.isTaskStructure()) {
            return;
        }
        JsonObject json = TaskJsonFactory.simpleTask(parsed.entryId(), parsed.type(), structure.trim(), "pyramid");
        EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
        state.questDetails.questDetailsPickTarget = "";
    ...
    static void applyBlockPick(Player player, TabletUiState state, String block) {
        ...
        if (!ModalTargetState.requireParts("task_block", parsed, 4) || !parsed.isTaskBlock()) {
            return;
        }
        String blockId = block.trim();
        JsonObject json = TaskJsonFactory.simpleTask(parsed.entryId(), parsed.type(), blockId, QuestTaskIconActions.blockIcon(blockId));
        EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
        state.questDetails.questDetailsPickTarget = "";
    ...
    static void applyStatPick(Player player, TabletUiState state, String stat) {
        ...
        if (!ModalTargetState.requireParts("task_stat", parsed, 4) || !parsed.isTaskStat()) {
            return;
        }
        String statTarget = stat.trim();
        JsonObject json = TaskJsonFactory.simpleTask(parsed.entryId(), parsed.type(), statTarget, QuestTaskIconActions.statIcon(statTarget));
        EditorQuestCommandClient.putQuestTaskJson(player, parsed.questId(), json.toString());
        state.questDetails.questDetailsPickTarget = "";
    ...
    static void applyDimensionPick(Player player, TabletUiState state, String dimension) {
        ...
        if (!ModalTargetState.requireParts("task_dimension", parsed, 4) || !parsed.isTaskDimension()) {
            return;
        }
        JsonObject json = TaskJsonFactory.defaultTask(parsed.entryId(), "location");
        json.addProperty("type", parsed.type());
    ...
    static void applyLootTablePick(Player player, TabletUiState state, String lootTable) {
        ...
        if (!ModalTargetState.requireParts("task_loot_table", parsed, 3) || !parsed.isRewardLootTable()) {
            return;
        }
        ...
        JsonObject json = QuestTaskLootTableRewardEditor.isLootTable(existing)
                ? existing.deepCopy()
                : TaskJsonFactory.defaultReward(parsed.entryId(), TaskJsonFactory.typePath(type));
        json.addProperty("id", parsed.entryId());
        json.addProperty("type", type);
        json.remove("fallback_item");
        json.remove("amount");
        json.addProperty("title", QuestTaskLootTableRewardEditor.displayName(lootTable));
        if (!json.has("icon")) {
