...
public final class ContextMenuPanel {
    ...
    public static WidgetGroup build(
        ...
        return build(x, y, w, actions, start, visibleRows, borderColor, state, afterAction, ContextMenuAnimationBridge.DEFAULT_KEY);
    }
    ...
    public static WidgetGroup build(
        ...
        return build(x, y, w, actions, start, visibleRows, borderColor, state, afterAction, ContextMenuAnimationBridge.DEFAULT_KEY, maxW, maxH);
    }
    ...
    public static WidgetGroup build(
        ...
        return buildInternal(x, y, w, actions, start, visibleRows, borderColor, state, afterAction, ContextMenuAnimationBridge.DEFAULT_KEY, true, maxW, maxH, x, y, scrollState, refresh);
    }
    ...
    private static WidgetGroup buildInternal(
        ...
        return animate ? ContextMenuAnimationBridge.wrap(menu, state, animationKey) : menu;
    }
    ...
    private static void addPromotedBar(WidgetGroup menu, List<ContextAction> promoted, int menuW, TabletUiState state, Consumer<ContextAction> afterAction, String animationKey) {
        ...
        for (int i = 0; i < visible; i++) {
            ContextAction action = visiblePromoted.get(i);
            int buttonX = promotedButtonX(menuW, visible, i);
            menu.addWidget(IconOnlyButton.create(buttonX, y, PROMOTED_BUTTON, ContextMenuRenderer.contextIconFileKey(action.icon()), action.accentColor(), click -> runAction(state, afterAction, action, animationKey))
    ...
    private static void runAction(TabletUiState state, Consumer<ContextAction> afterAction, ContextAction action, String animationKey) {
        ...
        ContextMenuAnimationBridge.finish(state, animationKey);
        action.action().run();
        if (afterAction != null) {
