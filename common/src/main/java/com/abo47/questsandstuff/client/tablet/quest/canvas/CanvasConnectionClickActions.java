...
final class CanvasConnectionClickActions {
    ...
    static boolean handleQuickConnect(TabletUiState state, Player player, Runnable refresher, QuestCardLayout hit, int button) {
        ...
        if (!quickConnectActive && !state.canvas.quickConnectSourceQuestId.isBlank()) {
    ...
    static boolean handlePendingConnect(TabletUiState state, Player player, Runnable refresher, QuestCardLayout hit, int button) {
        if ((state.canvas.connectSourceQuestId.isBlank() && state.canvas.connectSourceQuestIds.isEmpty()) || button != 0) {
            return false;
        }
    ...
    static boolean handleConnectMode(CanvasViewport canvasViewport, TabletUiState state, Player player, Runnable refresher, QuestCardLayout hit, int button) {
        ...
        if (state.canvas.connectSourceQuestId.isBlank() || state.canvas.connectSourceQuestId.equals(hit.questId())) {
            ...
            state.canvas.connectSourceQuestIds.clear();
            state.canvas.connectSourceQuestIds.add(hit.questId());
            state.canvas.canvasSelection.questIds().clear();
            state.canvas.canvasSelection.questIds().add(hit.questId());
            refresher.run();
            return true;
