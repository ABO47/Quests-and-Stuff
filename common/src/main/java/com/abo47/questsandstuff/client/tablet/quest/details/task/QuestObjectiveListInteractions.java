...
final class QuestObjectiveListInteractions {
    ...
    static boolean deleteSelected(Player player, TabletUiState state, String questId) {
        if (!QuestDetailsEditController.canEdit(state) || state.questDetails.questDetailsSelectedObjectiveId.isBlank()) {
            return false;
        }
    ...
    static boolean moveSelected(Player player, TabletUiState state, String questId, int offset) {
        if (!QuestDetailsEditController.canEdit(state) || state.questDetails.questDetailsSelectedObjectiveId.isBlank() || offset == 0) {
            return false;
        }
