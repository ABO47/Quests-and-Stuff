...
final class QuestHudIconRenderer {
    ...
    static void draw(GuiGraphics graphics, String iconId, int x, int y, int size, int alpha) {
        ...
        if (texture == null) {
            if (CanvasModelPreviewRenderer.isModelAsset(icon)
                    && CanvasModelPreviewRenderer.renderModelAsset(graphics, x, y, size, size, icon)) {
            ...
            texture = QuestIconProvider.iconTexture(icon);
        }
